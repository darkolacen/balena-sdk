...
// TODO: This is a new model using hostapps as the source of truth, and is intended to replace the current `os.ts` model once hostapps devexp is improved in open-balena and bob.
...
const transformHostApps = (apps: Application[]) => {
	const res: DeviceTypeOsVersions = apps.reduce(
		(deviceTypeOsVersions: DeviceTypeOsVersions, hostApp) => {
			...
			const hostAppDeviceType = (hostApp.is_for__device_type as any)[0].slug;
